...
namespace platf::virtualhid {
  ...
  int alloc_gamepad(input_context_t &context, const gamepad_id_t &id, const gamepad_arrival_t &metadata, feedback_queue_t feedback_queue) {
    ...
    if (support.supports_motion) {
      // 250 Hz instead of upstream's 100: the client throttles to whatever we
      // request (and hardware-caps below it), and a shorter batching period
      // means fresher motion at every downstream sampling point — the
      // forwarded DS3 sixaxis path is latency-sensitive (see retro-stream
      // profiles/sixaxis.rs).
  ...
  int rebind_gamepad(input_context_t &context, const gamepad_id_t &id, feedback_queue_t feedback_queue) {
    ...
    if (gamepad->adapter->support().supports_motion) {
      // 250 Hz — see add_gamepad above.
      raise_feedback_unlocked(gamepad, gamepad_feedback_msg_t::make_motion_event_state(id.clientRelativeIndex, LI_MOTION_TYPE_ACCEL, 250));
      raise_feedback_unlocked(gamepad, gamepad_feedback_msg_t::make_motion_event_state(id.clientRelativeIndex, LI_MOTION_TYPE_GYRO, 250));
    }
